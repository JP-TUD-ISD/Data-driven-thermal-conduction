...
import math  # change for changing data set

# material parameters
K = 0.8  # thermal conductivity of the known domain ... W / (m K)
C = [620 * 2400, 990 * 2400, 300.0 * 2400, 300.0 * 2400, 300.0 * 2400]  # heat capacity per domain 

# changes for changing data set
weekins = 60 * 60 * 24 * 7
# 60 s/min * 60 min/h * 24 h/d * 7 d/weej

# generate conduction data
syntdata = False  # flag to use synthetically generated data
gfile = "g1.dat"
qfile = "q1.dat"
...
if syntdata:
    lowbound = [-1e2, -1e2, -1e2, -1e2]  # lower bound for lin space for each material
    upbound  = [1e2, 1e2, 1e2, 1e2]  # upper bound for lin space for each material
    nDB = [int(1e5+1), int(1E5+1), int(1E5+1), int(1E5+1)]
    ...
    for i in range(1, len(ga[0,:])):
        nDB.append(int(len(ga[:, i])))
    mats = len(C)
    mnDB = int(max(nDB))
    gData = np.zeros((mats, mnDB))
    qData = np.zeros((mats, mnDB))
    for i in range(mats):
        gData[i, 0:nDB[i]] = ga[:, i + 1]
        qData[i, 0:nDB[i]] = qa[:, i + 1]
...
# initial temp
inittemp = True
itemps = [10.47583, 11.82519, 6.86384, 6.813, 6.813]
...
if loadtype == 0:
    ...
    dt = 353.14634458378345  # time increment
    nSteps = tFinal / dt
    print(nSteps)
...
if bflag:
    ...
    for n in range(nSteps):
        ...
        # changes for changing data set
        tnn = n*dt
        week = math.floor(t/weekins)
        weekn = math.floor(tnn/weekins)
        
        if week == 1 and weekn == 0:
            # change data set first time
            if syntdata:
                print("some settings are wrong")
                exit()
            else:
                # first step regenerate data set arrays
                ga = np.loadtxt("g2.dat")
                qa = np.loadtxt("q2.dat")
                nDB = []
                for i in range(1, len(ga[0,:])):
                    nDB.append(int(len(ga[:, i])))
                mats = len(C)
                ...
                # second step, rebuild FE matrices
            ...
            if syntdata:
                ...
                # first step regenerate data set arrays
                ga = np.loadtxt("g3.dat")
                qa = np.loadtxt("q3.dat")
                nDB = []
                for i in range(1, len(ga[0,:])):
                    nDB.append(int(len(ga[:, i])))
                mats = len(C)
                ...
                # second step, rebuild FE matrices
                C = [310.0 * 2400, 300.0 * 2400, 330.0 * 2400, 330.0 * 2400, 550.0 * 2400]  # heat capacity per domain 
                # assemble FE matrices
                Bmat = np.zeros((N,N+1))
                Cmat = np.zeros((N+1,N+1))
                Kref = np.zeros((N+1,N+1))
        ...
        while (iter < itMax):
            ...
            for e in range(New):  
                ...
                Rdd[New:2*New] -= dt*Dmat[e,1:-1]*Kdd[ei]*gData[ei, idx[n+1,e]]  # here New since one less node
